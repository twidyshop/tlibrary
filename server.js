...
// --- FUNGSI DETEKSI SUB-KATEGORI OTOMATIS KHUSUS EBOOK (DARI TLIBRARY) ---
...
// --- FITUR PRODUK DIGITAL & ADMIN (TLIBRARY) ---
...
// --- PENGGABUNGAN DATA PPOB (DIGIFLAZZ + HAYBI) DARI TWIDYSHOP ---
        ...
        if (digiUser && digiKey) {
            try {
                ...
                const mappedDigi = digiData.map(p => {
                    ...
                    const isHaybiTerritory = haybiTargetBrands.some(target => String(p.brand).toUpperCase() === target);
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    <url><loc>${baseUrl}/topup</loc><changefreq>daily</changefreq><priority>0.9</priority></url>
    <url><loc>${baseUrl}/riwayat</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>
    <url><loc>${baseUrl}/promo</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>
    <url><loc>${baseUrl}/panduan</loc><changefreq>monthly</changefreq><priority>0.7</priority></url>`;

    if (digitalProducts && digitalProducts.length > 0) {
        digitalProducts.forEach(p => {
            if (p.name) {
                const slug = generateSlug(p.name);
                xml += `<url><loc>${baseUrl}/${slug}</loc><changefreq>monthly</changefreq><priority>0.6</priority></url>`;
            }
        });
    }
